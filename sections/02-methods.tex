...
% - Data split
Next, we split the dataset sequence-wise with $80\%$ for training, $10\%$ for validating and $10\%$ for testing, ending up with 174,949 poses\todo{check numbering consistent} for training\todo{Datasetlength Train length?}. When we load one item of this set we create a ROI history, which is a temporal sequence of areas of highest activation. We defined the length of the history as 50 and the ROI is a $4\times4$ grid of the floor with the signal values for this frame. This grid is a squared approximation of the triangular SensFloor patches explained in section \ref{sec:introduction}; two fields that form a rectangle are transformed into a $2\times2$ square where the diagonal is the mean value of the fields
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{img/roi-visualization.pdf}
