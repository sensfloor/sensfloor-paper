...
% - Data split
Next, we split the dataset sequence-wise with $80\%$ for training, $10\%$ for validating and $10\%$ for testing, ending up with 174,949 poses\todo{check numbering consistent} for training\todo{Datasetlength Train length?}. When we load one item of this set we create a ROI history, which is a temporal sequence of areas of highest activation. We defined the length of the history as 50 and the ROI is a $4\times4$ grid of the floor with the signal values for this frame. This grid is a squared înterpolation of the triangular SensFloor patches explained in section \ref{sec:introduction}; two fields that form a rectangle are transformed into a $2\times2$ square where the diagonal is the mean value of the fields. Figure \ref{fig:roi} depicts this transformation.
\todo{confirm roi explanation} % maybe that's not so important but if we have space
% - Data (roi, history)
The history is created by going back 50 frames and for each frame taking a ROI with the signals received at this frame. When there are no signals in the frame we take an empty ROI containing only signals with the value 127
% - normalize: 0-1 and not -1 to 1 because most will be 0. e.g. 127
All signals are normalized to the range $[0, 1]$, where values of 127 will be 0 and values of 255 will be 1. So for parts where there are no signals, the model receives 0.
