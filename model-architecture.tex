...
\begin{tikzpicture}[
    ...
    % Styles
    tensor/.style={
        draw, fill=blue!10, rectangle, minimum height=1.7cm, minimum width=0.5cm, align=center, inner sep=2pt
    },
    convblock/.style={
        draw, fill=orange!10, rectangle, rounded corners, minimum height=1.7cm, minimum width=2.5cm, align=center, drop shadow
    },
    lstm/.style={
        draw, fill=green!10, rectangle, rounded corners, minimum height=1.7cm, minimum width=1.5cm, align=center
    },
    dense/.style={
    ...
    % Feature Vector 1
    \node[tensor, right=0.5cm of flat1, fill=red!10, minimum height=1.7cm, minimum width=0.3cm] (feat1) {\tiny 256};
    
    % LSTM Cell 1
    \node[lstm, right=0.8cm of feat1] (lstm1) {\textbf{LSTM}\\ \tiny (2 Layers)};

    % --- TIME STEP 2 (Dots) ---
    \node[below=0.3cm of in1] (dots_in) {$\vdots$};
    \node[below=0.3cm of cnn1] (dots_cnn) {$\vdots$};
    \node[below=0.3cm of feat1] (dots_feat) {$\vdots$};
    % \node[below=1.5cm of lstm1] (dots_lstm) {$\vdots$};

    % --- TIME STEP T (Last) ---
    % Input Frame T
    \node[tensor, fill=gray!20, label=above:{$x_{t_T}$}, below=1.5cm of in1] (inT) {\tiny $1\times12\times12$};
